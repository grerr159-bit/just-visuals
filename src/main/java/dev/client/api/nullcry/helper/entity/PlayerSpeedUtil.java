...
@Getter
@Setter
public class PlayerSpeedUtil implements ClientApi {
    ...
    public double calculateSpeed(final MovementEvent move, boolean damageBoost, boolean hasTime, boolean autoJump, float damageSpeed) {
    ...
    private float getAIMoveSpeed(final ClientPlayerEntity contextPlayer) {
        ...
        float speed = contextPlayer.getMovementSpeed() * 1.3f;
        contextPlayer.setSprinting(prevSprinting);
        return speed;
    }

    private float getFrictionFactor(final ClientPlayerEntity contextPlayer, final MovementEvent move) {
        BlockPos.Mutable blockpos$mutable = new BlockPos.Mutable();
        blockpos$mutable.set(move.getFrom().x, move.getAabbFrom().minY - 1.0D, move.getFrom().z);
